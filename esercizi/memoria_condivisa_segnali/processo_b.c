...
sigset_t signal_set;
volatile int can_write =0;


void* signal_thread(void* arg) {
    int received_signal;
    
    printf("Processo B - Signal thread avviato\n");
    ...
    while (1) {
        ...
        switch (received_signal) {
            case SIGUSR1:
                ...
                break;
            case SIGTERM:
                printf("Processo B - Signal thread termina\n");
                return NULL;
        }
    }
}
...
void* writer_thread(void* arg) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        printf("Processo B: scritto '%c' in pos %d\n", ch, shared_data->write_index);
        shared_data->write_index++;
        
        can_write = 0;  // Resetta permesso
        
        // Invia segnale a B
          // CORRETTO: Controlla PID di A, non di B
        if (shared_data->process_a_pid > 0) {
            printf("Processo B: Invio USR2 a processo A (PID: %d)\n", 
                   shared_data->process_a_pid);
            kill(shared_data->process_a_pid, SIGUSR2);
        } else {
            printf("Processo B: ERRORE - PID di A = %d\n", shared_data->process_a_pid);
        }
...
int main(void){
    ...
    printf("Processo B avviato (PID: %d)\n", getpid());
 ...
 // ===== INZIALIZZAZIONE DELLA STRUCT =====//
    ...
    shared_data->ready_flag = 1;  // Segnala che B è pronto
    printf("Processo B: Il mio PID = %d, PID di A = %d\n", 
