/*
 * profile_thread.c - Profiling prestazioni usando thread
 * Crea 10 thread che eseguono sequenzialmente funzione dummy
...
typedef struct {
    ...
    TimeProfile *profile;   /* Puntatore a struttura risultati */
    pthread_mutex_t *mutex; /* Mutex per sincronizzazione output */
} ThreadData;

/* Funzione dummy che simula lavoro computazionale identica a fork version */
void dummy_function(int thread_id) {
    int i, j;
    volatile double result = 0.0;
    volatile int array[1000];
    
    printf("  Thread %d: Inizio esecuzione dummy\n", thread_id);
    ...
    for (i = 0; i < DUMMY_WORK_SIZE; i++) {
        ...
        /* Simulazione system call occasionale */
        if (i % 100000 == 0) {
    ...
    printf("  Thread %d: Dummy completato (risultato: %.2f)\n", 
           thread_id, result);
}
...
/* Misura tempi di esecuzione per un singolo thread */
TimeProfile measure_single_thread(int iteration, pthread_mutex_t *mutex) {
    pthread_t thread;
    ...
    /* Prepara dati per il thread */
    thread_data.thread_id = iteration + 1;
    thread_data.profile = &profile;
    thread_data.mutex = mutex;
    
    /* Crea thread */
    result = pthread_create(&thread, NULL, thread_function, &thread_data);
    if (result != 0) {
        fprintf(stderr, "Errore creazione thread %d: %s\n", 
                iteration + 1, strerror(result));
        exit(EXIT_FAILURE);
    }
    ...
    printf("  Thread %d terminato con successo\n", iteration + 1);
...
void print_summary_statistics(TimeProfile profiles[], int count) {
    ...
    if (total.wall_time > 0) {
        printf("Overhead thread creation/join: %.3f s (%.1f%%)\n", 
               total.wall_time - (total.user_time + total.system_time),
               ((total.wall_time - (total.user_time + total.system_time)) / total.wall_time) * 100);
    }
    
    /* Confronto teorico con esecuzione sequenziale */
    printf("Esecuzione sequenziale stimata: %.3f s\n", avg.wall_time * count);
    printf("Speedup ottenuto: %.2fx\n", (avg.wall_time * count) / total.wall_time);
}
...
int main() {
    ...
    printf("=== PROFILING PRESTAZIONI CON THREAD ===\n");
    printf("Esecuzioni programmate: %d\n", NUM_ITERATIONS);
    printf("Dimensione lavoro dummy: %d iterazioni\n", DUMMY_WORK_SIZE);
    printf("PID processo principale: %d\n", getpid());
    
    /* Inizializza mutex per output sincronizzato */
    result = pthread_mutex_init(&output_mutex, NULL);
    if (result != 0) {
        fprintf(stderr, "Errore inizializzazione mutex: %s\n", strerror(result));
        return EXIT_FAILURE;
    }
    
    /* Verifica supporto RUSAGE_THREAD */
#ifndef RUSAGE_THREAD
    printf("ATTENZIONE: RUSAGE_THREAD non supportato, usando RUSAGE_SELF\n");
#endif
    
    printf("Modalità: Esecuzione sequenziale thread\n\n");
    
    /* Misura tempo totale inizio */
    total_start_clock = clock();
    clock_gettime(CLOCK_REALTIME, &total_start_wall);
    
    /* Esegue tutte le iterazioni sequenzialmente */
    for (i = 0; i < NUM_ITERATIONS; i++) {
        profiles[i] = measure_single_thread(i, &output_mutex);
    }
    ...
    printf("\nProfiling con thread completato.\n");
    return EXIT_SUCCESS;
}